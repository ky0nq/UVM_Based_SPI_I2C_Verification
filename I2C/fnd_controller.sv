...
module fnd_controller (
    ...
    logic w_1khz;                                           
    logic [7:0] fnd_val;                                    // FND data Capture
    logic [1:0] digit_sel;                                  // Display Period
    logic [3:0] selected_digit;                             // choice Display Digit  
    logic [3:0] digit_1, digit_10, digit_100, digit_1000;   // displays Digit (rx_data)
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            fnd_val <= rx_data;  // Capture stable rx_data on Done pulse
        end
    end
...
module clk_div_1khz (
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (cnt == 100_000 - 1) begin   // 100MHz / 100_000Hz = 1kHz toggle period
                cnt    <= 17'd0;
                o_1khz <= 1'b1;
            end else begin
                cnt    <= cnt + 1'b1;
